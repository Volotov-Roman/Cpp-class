...
class ClearAllShapes : public Command
{
private:
    ShapePool& mPool;

public:
    ClearAllShapes(ShapePool& pool) : mPool(pool) {}
...
int main()
{
    ...
    control.addButton(new Button({40, 440, 240, 40}, "Clear All Shapes"), new ClearAllShapes(pool));
    control.addButton(new Button({40, 500, 240, 40}, "Random Colors & Pos"), new MultiCommand(std::vector<Command*> {new RandomAllColorsCommand(pool), new RandomAllPositionsCommand(pool)}));
