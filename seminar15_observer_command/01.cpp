...
class Triangle : public Observer, public Drawable
{
protected:
    Vector2 mStart;
    Vector2 mCenter;
    float mRadius;
    Color mColor {140, 0, 255, 255};

public:
    Triangle(Vector2 position, float radius)
        : mStart(position), mCenter(position), mRadius(radius)
    {
    }

    void draw() const override
    {
        DrawPoly(mCenter, 3, mRadius, 0, mColor);
    }

    void update(float value) override
    {
        mCenter.y = mStart.y - value * 2.0f;
    }
};
