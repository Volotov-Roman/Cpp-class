...
public:
    ...
    void execute() override
    {
        for (size_t i = 0; i < mPool.getNumbeorOfShapes(); ++i)
        {
            ...
            Vector2 pos = {mPool.getPosition().x + getRandomFloat(min.x, max.x), 
...
public:
    ...
    void execute() override
    {
        ...
        if (r == 0)
        {
            ps = std::make_unique<RectangleShape>(
                Vector2{getRandomFloat(mMinSize, mMaxSize), getRandomFloat(mMinSize, mMaxSize)}
            );
        }
        else if (r == 1)
        {
            ps = std::make_unique<CircleShape>(getRandomFloat(mMinSize / 2, mMaxSize / 2));
        }
        else
        {
            ps = std::make_unique<CircleShape>(getRandomFloat(mMinSize / 2, mMaxSize / 2), 3);
        }
        ...
        Vector2 pos = {mPool.getPosition().x + getRandomFloat(0, mPool.getSize().x - size.x), 
                      mPool.getPosition().y + getRandomFloat(0, mPool.getSize().y - size.y)};
        ps->setPosition(pos);
