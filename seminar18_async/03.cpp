...
uint64_t func(std::vector<uint64_t>::const_iterator start, std::vector<uint64_t>::const_iterator end)
{
...
uint64_t getMax(int n, const std::vector<uint64_t>& v)
{
    ...
    for (int i = 0; i<n; i++)
    {
        ...
        std::packaged_task<uint64_t(std::vector<uint64_t>::const_iterator, std::vector<uint64_t>::const_iterator)> task {func};
        futures.push_back(task.get_future());
        threads.push_back(std::thread {std::move(task), begin, end});
    }
